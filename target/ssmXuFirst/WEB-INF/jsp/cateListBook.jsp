...
<html>
  ...
  <jsp:include page="menu.jsp"></jsp:include>
		<c:forEach items="${categoryList}" var="cate">
			<a href="<%=basePath%>findFirCateOfBook/${cate.id}">${cate.catename}</a>
		</c:forEach>
  <br/>
	  <table>
	    ...
	    <tr>
			<c:forEach items="${pageBean.beanList}" var="book" varStatus="mar">
				<td>
					<img src='<%=picPath%>${book.bookimage}' alt='${book.bookname}' width="330px" height="130px">
					<br/>
					<a href='<%=basePath%>findTheBook/${book.id}'>${book.bookname}</a>
					&nbsp;&nbsp;&nbsp;著:${book.author}
				</td>
				<td>
					<textarea cols="20" rows="9" readonly="readonly">${book.intro}</textarea>
				</td>
				<c:choose>
	   ...
	   <a href='<%=basePath%>findFirCateOfBook/${uCateId}' >首页</a>
			...
			<c:forEach var="i" begin="${begin}" end="${end}">
				<c:choose>
					...
					<c:otherwise>
						<a href="<%=basePath%>findFirCateOfBook/${uCateId}?pc=${i}" >${i}</a>
					</c:otherwise>
				</c:choose>
			</c:forEach>
   	<a href='<%=basePath%>findFirCateOfBook/${uCateId}?pc=${pageBean.tp}' >尾页</a>
  </div>
  </body>
</html>
